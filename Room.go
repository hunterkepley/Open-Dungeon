...
func generateRooms(doneRooms chan bool, amount int, startPos pixel.Vec, max pixel.Vec, min pixel.Vec) {
	for i := 0; i < amount; i++ {
		...
		rY := startPos.Y
		if i != 0 {
			rX = randFloat64(rooms[i-1].pos.X-max.X*2, (rooms[i-1].pos.X+rooms[i-1].size.X)+max.X)
			rY = randFloat64(rooms[i-1].pos.Y-max.Y*2, (rooms[i-1].pos.Y+rooms[i-1].size.Y)+max.Y)
		}
